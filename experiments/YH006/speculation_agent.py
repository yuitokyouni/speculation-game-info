...
**設計 A: MARKET_ORDER + 次 step 冒頭で self-cancel** (SPEC §3.2(c) 準拠):
- open / close は常に MARKET_ORDER で送る
...
- これにより 1 SG あたり同時 outstanding MARKET_ORDER は最大 1 に抑制
- ただし他 SG の stale が反対板に残る間は PAMS の remain_executable_orders
  slow path (O(N²)) が発火しうる。実測で判断する。
